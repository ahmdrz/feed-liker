...
class Instagram:
    ...
    def login(self, force=False):
        if not self.logged_in or force:
            response = self.send_request('si/fetch_headers/?challenge_type=signup&guid=', login=True)
            if response.status_code == 200:
                ...
                response = self.send_request('accounts/login/', self.generate_signature(json.dumps(data)), True)
                if response.status_code == 200:
                    json_result = json.loads(response.content)

                    self.logged_in = True
                    self.username_id = json_result["logged_in_user"]["pk"]
    ...
    def logout(self):
        response = self.send_request("accounts/logout/")
    ...
    def generate_signature(self, data):
        ...
        signature = "ig_sig_key_version={}&signed_body={}.{}"
        return signature.format(self.SIG_KEY_VERSION, sign, urllib.pathname2url(data))

    def followings(self, max_id=None, user_id=None):
        if user_id is None:
            user_id = self.username_id
        query = "friendships/{}/following/?ig_sig_key_version={}&rank_token={}"
        if max_id:
            query += "&max_id={}".format(max_id)
        query = query.format(user_id, self.IG_SIG_KEY, self.rank_token)
        response = self.send_request(query)
        return json.loads(response.content)

    def user_feeds(self, max_id=None, user_id=None):
        if user_id is None:
            user_id = self.username_id
        query = "feed/user/{}/?ig_sig_key_version={}&rank_token={}&ranked_content=true"
        if max_id:
            query += "&max_id={}".format(max_id)
        query = query.format(user_id, self.IG_SIG_KEY, self.rank_token)
        response = self.send_request(query)
    ...
    def timeline(self, max_id=None):
        query = "feed/timeline/?rank_token={}&ranked_content=true"
        if max_id:
            query += "&max_id={}".format(max_id)
        response = self.send_request(query)
        return json.loads(response.content)

    def followers(self, max_id=None, user_id=None):
        if user_id is None:
            user_id = self.username_id
        query = "friendships/{}/followers/?ig_sig_key_version={}&rank_token={}"
        if max_id:
            query += "&max_id={}".format(max_id)
        query = query.format(user_id, self.IG_SIG_KEY, self.rank_token)
        response = self.send_request(query)
    ...
    def get_username(self, username):
        response = self.send_request("users/{}/usernameinfo".format(username))
        return json.loads(response.content)
    ...
    def get_user(self, user_id=None):
        ...
        response = self.send_request("users/{}/info".format(user_id))
        return json.loads(response.content)
    ...
    def like(self, media):
        ...
        response = self.send_request('media/{}/like/'.format(media), self.generate_signature(data))
        return json.loads(response.content)

    def send_request(self, endpoint, post=None, login=False):
        if not self.logged_in and not login:
